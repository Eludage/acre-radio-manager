...
| `DEST` | Output path for the packed `.pbo` (should end in `AcreRadioManager.pbo`) |
...
| `DEPLOYDIR` | Target `addons` folder of your local `@AcreRadioManager` mod |
...
- Controls (idc → control name — brief purpose):
  - Section titles (no idc, -1)
    ...
    - RadioSavestatesTitle — section title "Radio Savestates"
  - Background panels (no idc, -1)
    ...
    - RadioSavestatesBackground — radio savestates section background
    - RadioSavestatesButtonsBackground — radio savestates buttons background
  ...
  - Radio Savestates controls
    - 16030 → RadioSavestatesGroup — control group for savestate list
...
|---|---|---|---|
| `ARM_RscButton` | `COLOR_BLACK_50` | `COLOR_BLACK_100` | Base class (not used directly for dynamic controls) |
